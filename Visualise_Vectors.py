...
def getBezier(p1, p2, rp, t):
    np1 = linearInterpolation(p1, rp, t)
    np2 = linearInterpolation(rp, p2, t)
    return linearInterpolation(np1, np2, t)

def getF(t):
    # bezier with one point
    bar = t * len(drawpoints)
    ni = math.floor(bar)
    prog = bar - ni
    if (ni + 2 < len(drawpoints)):
        if (ni % 2 == 0):
            prog /= 2
        else:
            ni -= 1
            prog = prog / 2 + 0.5
        return getBezier(drawpoints[ni], drawpoints[ni + 2], drawpoints[ni + 1], prog)
    else:
...
while running:
    ...
    if len(path_points) > 1:
        pygame.draw.lines(screen, YELLOW, False, path_points, path_thickness)  # Draw yellow line
