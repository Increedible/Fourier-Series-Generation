...
paths, attributes = svg2paths("Mickey_Mouse.svg")
...
with open(filename, 'w') as file:
    ...
    for n in range(nmin, nmax + 1):
        ...
        fullformulax += f"{r}cos({phi} + 2 * \\pi\\ * {n} * t) +"
        fullformulay += f"{r}sin({phi} + 2 * \\pi\\ * {n} * t) +"
with open("desmosformula.txt", 'w') as file:
    fullformulax = fullformulax[:len(fullformulax) - 2]
    fullformulay = fullformulay[:len(fullformulay) - 2]
    print("\n\n------------------\n")
    print(f"({fullformulax}, {fullformulay})")
    file.write(f"({fullformulax}, {fullformulay})")
